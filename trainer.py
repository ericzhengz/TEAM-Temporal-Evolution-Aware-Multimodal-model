...
def _train(args):
    ...
    for task in range(data_manager.nb_tasks):
        ...
        # 在trainer.py的model.after_task()后添加
        ...
        logging.info("Top-5 准确率: {:.2f}%".format(cnn_accy["top4"]*100))

        # 更新评估曲线
        cnn_curve["top1"].append(cnn_accy["top1"])
        cnn_curve["top5"].append(cnn_accy["top4"])
        ...
        # 如果存在虫态距离矩阵，每个任务结束后可视化一次
        if hasattr(model, 'state_distance'):
            vis_path = f"{logs_name}/task{task}_state_distance.png"
            try:
                model.state_distance.visualize_distance_matrix(save_path=vis_path)
                logging.info(f"已保存虫态距离矩阵可视化: {vis_path}")
            except Exception as e:
                logging.error(f"虫态距离矩阵可视化失败: {str(e)}")
